    ...
    <div class="bg-white dark:bg-gray-900 rounded-3xl p-8 max-w-md w-full shadow-2xl relative transform transition-all duration-300 scale-100 animate-in fade-in-0 zoom-in-95">
      ...
      <form @submit.prevent="$emit('submit', newGroup)" class="space-y-6">
        ...
        >
          ...
          <span v-if="isCreating" class="relative flex items-center justify-center text-lg">
      ...
      <div v-if="errorMessage" class="mt-6 p-4 bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 rounded-2xl animate-in slide-in-from-top-2 duration-300">
